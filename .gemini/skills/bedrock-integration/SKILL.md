...
This skill provides a self-contained test environment for automatically verifying whether the Bedrock tool calling loop works. This allows the AI agent to iteratively debug and patch the `bedrockNovaProvider.ts` abstraction layer independently, without requiring the user to act as an intermediary tester.

## Behavior-Driven Development (BDD) Protocol

When working on the Bedrock provider integration, we strictly adhere to a **Behavior-Driven Development (BDD)** workflow:

1. **Expected Behavior First (Evals):** Prior to making code adjustments, we write or modify a behavioral evaluation file under `/evals` (such as `evals/bedrock-plan-mode.test.ts`) that asserts how Bedrock should select tools or formats in response to user prompts.
2. **Fail First (Red):** Run `vitest evals/bedrock-plan-mode.test.ts` (or the direct provider script) to observe the failure, establishing a clear baseline contract.
3. **Implement & Fix (Green):** Implement changes in `packages/core/src/core/providers/bedrockNovaProvider.ts` to translate and conform Nova's payloads to the expected Gemini core standard.
4. **Verification:** Confirm that the evaluation passes 100% locally before submitting a PR.
...
An automated ESM script that directly instantiates `BedrockNovaContentGenerator` and drives a full multi-turn conversational tool call loop. This is the most reliable way to test the Bedrock integration because it completely avoids TTY/PTY allocation issues.
...
```
This script will:
1. Direct-call `BedrockNovaContentGenerator` with `write_file` and `update_topic` tool definitions.
...
2. Read `bedrockNovaProvider.ts` to map and parse exactly what Nova generates.
