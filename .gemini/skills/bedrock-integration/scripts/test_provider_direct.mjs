...
async function run() {
    ...
    const request = {
        ...
        contents: [
            {
                role: 'user',
                parts: [{ text: 'Create the file test-isolated.txt with the content "Hello from Bedrock"' }]
            }
        ],
        config: {
    ...
    try {
        ...
        for await (const chunk of stream) {
            if (chunk.functionCalls) {
                console.log('Model generated tool calls:', JSON.stringify(chunk.functionCalls, null, 2));
                    ...
                    parts: [
                        {
                            functionResponse: {
                                name: call.name,
                                response: { output: 'Successfully wrote file test-isolated.txt' },
                                id: call.id
                            }
                        }
                    ]
