...
async function runInteractiveTest() {
  ...
  const ptyProcess = spawn(
    process.execPath,
    ['bundle/gemini.js', '--model', 'bedrock/nova-lite'],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
  ...
  ptyProcess.stdout.on('data', (data) => {
    ...
    if (stripped.includes('>') && !commandSent) {
      ...
      ptyProcess.stdin.write(
        `Create the file ${testFileName} with the text "Hello from Bedrock"\n`,
      );
    }

    if (
      stripped.toLowerCase().includes('api error') ||
      stripped.includes('FAILURE') ||
      stripped.includes('Expected toolResult blocks')
    ) {
